...
#!/usr/bin/env python3
# Juego En|Par| (Creado por T) - Versión Final Portable + A11y
...
# Corrección de URLs usando f-strings
...
def crear_juego(filas, columnas):
    # Versión optimizada: Sin Numpy (Listas nativas)
    ...
    for i in range(filas):
        inicio = i * columnas
        fin = inicio + columnas
        tablero.append(cartas[inicio:fin])
    return tablero

def crear_matriz_visibles(filas, columnas):
    # Crea una matriz de False del tamaño filas x columnas (Sin Numpy)
...
def mostrar_tablero(visibles, cartas, header=None):
    ...
    # --- LÓGICA MODO ACCESIBLE (TEXTO) ---
    if MODO_ACCESIBLE:
        print("\n--- ESTADO DEL TABLERO (Modo Texto) ---")
        filas = len(cartas)
        columnas = len(cartas[0])
        for i in range(filas):
            for j in range(columnas):
                estado = f"{cartas[i][j]}" if visibles[i][j] else "[OCULTA]"
                print(f"Fila {i}, Columna {j}: {estado}")
        print("---------------------------------------\n")
        return
...
def jugar_versus(nombres):
    ...
    while encontradas < total_parejas:
        ...
        def ver_carta1():       # def para indicar que estas en la carta 1
            turno_base()
            if MODO_ACCESIBLE:
                print(f"--> {p['name']} está eligiendo la Carta 1")
            else:
                print("--> Carta 1")
        ...
        def ver_carta2():            # otro def para mostrarte la carta
            turno_base()
            if MODO_ACCESIBLE:
                print(f"--> Carta 1 revelada: {cartas[f1][c1]}")
                print(f"--> {p['name']} está eligiendo la Carta 2")
            else:
...
def jugar_solitario():
    ...
    while encontradas < total_parejas:
        ...
        def redibujar_c2():            # def igual que para la carta 1 peoro ahora para la casrta 2
            ...
            dibujar_pantalla()
            if MODO_ACCESIBLE:
                print(f"--> Carta 1 es: {cartas[f1][c1]}")
                print("--> Selecciona Carta 2")
            else:
                print(f"--> Carta 1: {cartas[f1][c1]}")
                print("--> Carta 2")
...
if __name__ == '__main__':
    ...
    # --- PREGUNTA DE ACCESIBILIDAD AL INICIO ---
    limpiar_pantalla()
    print("CONFIGURACIÓN DE ACCESIBILIDAD (A11y)")
    print("¿Deseas activar el Modo Texto para lectores de pantalla?")
    resp_acc = input("[S] Sí / [Enter] No: ").strip().upper()
    if resp_acc == 'S':
        MODO_ACCESIBLE = True
        print(">> Modo Accesible ACTIVADO.")
    else:
        print(">> Modo Visual Estándar ACTIVADO.")
    time.sleep(1.5)
